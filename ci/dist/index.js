...
function preCommit(props) {
    const actionYmlPath = path_1.default.resolve(props.workspace, 'action.yml');
    const fileContent = fs_1.default.readFileSync(actionYmlPath, 'utf-8');
    const content = yaml_1.default.parse(fileContent);
    const { image: oldImage } = content.runs;
    const tag = oldImage.split(':').pop();
    const newImage = oldImage.replace(tag, props.tag);
    const newYmlContent = yaml_1.default.stringify({
        ...content,
        runs: {
            ...content.runs,
            image: newImage,
